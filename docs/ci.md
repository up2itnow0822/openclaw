...
| `checks-node-compat-node24`      | Node 24 floor build and smoke lane                                                           | Manual CI dispatch for releases    |
...
Node 24 floor smoke, `check`, `check-additional`, build smoke, docs checks,
...
CI routing-only edits, selected cheap core-test fixture edits, and narrow plugin contract helper/test-routing edits use a fast Node-only manifest path: preflight, security, and a single `checks-fast-core` task. That path avoids build artifacts, Node 24 floor smoke, channel contracts, full core shards, bundled-plugin shards, and additional guard matrices when the changed files are limited to the routing or helper surfaces that the fast task exercises directly.
...
| `blacksmith-4vcpu-ubuntu-2404`   | `CodeQL Critical Quality`, lower-weight extension shards, `checks-fast-core`, `checks-node-compat-node24`, `check-prod-types`, and `check-test-types`                                                                                                                                                                                                                                                                                                                   |
